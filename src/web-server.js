...
// Configuration
let PORT = process.env.WEB_PORT || 4001;
...
let qrUpdateTimeout = null; // Debounce QR updates
        ...
        if (eventType === 'change') {
          try {
            ...
            if (newTimestamp !== lastQRTimestamp) {
              ...
              console.log('📱 QR file changed, debouncing update...');
              ...
              // Debounce QR updates to prevent rapid emissions
              qrUpdateTimeout = setTimeout(() => {
                try {
                  ...
                  console.log('✅ QR code update broadcasted to all clients (debounced)');
                } catch (error) {
                  console.error('❌ Error reading QR data for update:', error);
                }
              }, 1000); // Wait 1 second before emitting update
            }
          } catch (error) {
...
function forceQRRefresh() {
  try {
    ...
    if (fs.existsSync(qrFilePath)) {
      ...
      }, 500); // Shorter delay for forced refresh
...
app.get('/api/chat-logs', requireAuth, async (req, res) => {
  try {
    const { page = 1, limit = 50, chatId, phoneNumber, bookingId, hours = 168 } = req.query; // Changed default from 24 to 168 hours (7 days)
    ...
    // Time filter - use same 7-day default as stats
...
// Get single concierge by ID
app.get('/api/concierges/:id', requireAuth, async (req, res) => {
  try {
    const { models } = require('./models');
    const { id } = req.params;

    const concierge = await models.Concierge.findById(id)
      .populate('uplineId', 'name email conciergeId')
      .lean();
...
// Get WhatsApp instances (no auth required for dashboard status)
app.get('/api/whatsapp/instances', (req, res) => {
  ...
  if (fs.existsSync(connectionStateFile)) {
    try {
      ...
      // Check if state is recent (within last 10 minutes for connected state, 5 minutes for others)
      const stateAge = Date.now() - new Date(connectionState.lastUpdate).getTime();
      const isStateRecent = connectionState.connectionState === 'connected' ? 
        stateAge < 600000 : // 10 minutes for connected state
        stateAge < 300000;  // 5 minutes for other states
  ...
  const mainInstance = {
    ...
    attempt: connectionState?.connectionAttempts || 0,
    maxRetries: connectionState?.maxConnectionAttempts || 10
  };
        ...
        if (fs.existsSync(connectionStateFile)) {
          try {
            ...
            if (stateAge < 300000) { // 5 minutes
              mainInstance.statusMessage = 'Bot process active - QR generation disabled, use manual trigger';
              // QR generation is now manual only - no automatic triggering
            } else {
...
app.get('/api/whatsapp/bot-status', requireAuth, (req, res) => {
  try {
    const botStatus = {
      exists: !!global.mainBot,
      hasForceQR: !!(global.mainBot && typeof global.mainBot.forceQRGeneration === 'function'),
      isConnected: !!(global.mainBot && global.mainBot.isConnected),
      isConnecting: !!(global.mainBot && global.mainBot.isConnecting),
      connectionAttempts: global.mainBot ? global.mainBot.connectionAttempts : null,
      maxConnectionAttempts: global.mainBot ? global.mainBot.maxConnectionAttempts : null
    };
...
app.post('/api/whatsapp/qr/generate', requireAuth, async (req, res) => {
  try {
    ...
    // Method 1: Try to trigger QR generation directly from bot instance
    console.log('🔍 Checking bot instance status...');
    console.log('🔍 global.mainBot exists:', !!global.mainBot);
    console.log('🔍 global.mainBot.forceQRGeneration exists:', !!(global.mainBot && typeof global.mainBot.forceQRGeneration === 'function'));
    
    if(global.mainBot && typeof global.mainBot.forceQRGeneration === 'function') {
      try {
        console.log('🤖 Bot instance found, forcing disconnect and QR generation...');
        
        // First, disconnect the bot if it's connected
        if (global.mainBot.isConnected && global.mainBot.sock) {
          console.log('🔌 Disconnecting bot first...');
          try {
            await global.mainBot.sock.logout();
            global.mainBot.isConnected = false;
            global.mainBot.isConnecting = false;
            console.log('✅ Bot disconnected successfully');
          } catch (disconnectError) {
            console.log('⚠️ Error disconnecting bot:', disconnectError.message);
          }
        }
        
        // Clear session to force fresh QR generation
        const sessionDir = path.join(__dirname, '../data/whatsapp-session');
        if (fs.existsSync(sessionDir)) {
          try {
            // Cross-platform directory deletion with retry logic
            await deleteDirectoryWithRetry(sessionDir);
            console.log('🧹 Session directory cleared successfully');
          } catch (cleanupError) {
            console.log('⚠️ Session cleanup failed:', cleanupError.message);
            // Continue anyway - bot will handle it
          }
        }
        ...
        global.mainBot.connectionAttempts = 0;
        global.mainBot.qrAttempts = 0;
        
        // Force QR generation using the correct method
        console.log('🔄 Forcing QR generation...');
        const qrResult = await global.mainBot.forceQRGeneration();
        console.log('🔄 QR generation result:', qrResult);
        
        // Wait a moment for QR to be generated (increased wait time)
        ...
        if (fs.existsSync(qrFilePath)) {
          ...
          return res.json({
            success: true,
            message: 'QR code generated successfully via bot instance',
            qrData: qrData
          });
        }
      } catch (botError) {
        console.log('⚠️ Bot instance QR generation failed, trying fallback method:', botError.message);
      }
    }

    // Method 2: Clear session files to force QR generation on next bot restart
    ...
    if(fs.existsSync(sessionDir)){
      try{
        ...
        // Don't return error - continue with QR generation attempt
        console.log('⚠️ Continuing with QR generation despite cleanup error');
        qrGenerated = true;
      }
    } else {
    ...
    let attempts = 0;
    const maxAttempts = 20; // Increased from 15 to 20 for better reliability
    const checkQRFile = () => {
        ...
        const checkInterval = setInterval(() => {
          ...
          try {
            ...
            if (fs.existsSync(qrFilePath)) {
              ...
              clearInterval(checkInterval);
              console.log(`❌ QR file not found after ${maxAttempts} attempts`);
              console.log(`📁 Checking for QR file at: ${qrFilePath}`);
              console.log(`📁 Data directory exists: ${fs.existsSync(path.dirname(qrFilePath))}`);
              resolve({ success: false, error: 'QR file not found after maximum attempts' });
            }
          } catch (err) {
    ...
    if (result.success) {
      ...
      // Fallback: Create a simple QR code if bot instance is not available
      console.log('🔄 Bot instance not available, creating fallback QR...');
      try {
        const QRCode = require('qrcode');
        const fallbackQR = `WhatsApp Bot - Session cleared at ${new Date().toISOString()}`;
        ...
        return res.json({
          success: true,
          message: 'Fallback QR code generated successfully',
          qrData: qrData
        });
      } catch (fallbackError) {
    ...
    return res.json({
      success: true,
      message: 'Session cleared successfully. Bot will generate new QR code on next connection attempt.',
      qrData: null,
...
// PM2 Restart Endpoints
const { exec } = require('child_process');
const util = require('util');
const execAsync = util.promisify(exec);

// Force status synchronization endpoint
app.post('/api/whatsapp/sync-status', requireAuth, async (req, res) => {
  try {
    console.log('🔄 Forcing status synchronization...');
    
    if (global.mainBot && typeof global.mainBot.forceStatusSync === 'function') {
      const syncResult = await global.mainBot.forceStatusSync();
      ...
      if (syncResult) {
        ...
        // Provide more detailed information about why sync failed
        const debugInfo = {
          hasSocket: !!(global.mainBot.sock),
          isConnected: global.mainBot.isConnected,
          hasUser: !!(global.mainBot.sock && global.mainBot.sock.user),
          hasUserId: !!(global.mainBot.sock && global.mainBot.sock.user && global.mainBot.sock.user.id),
          wsReadyState: global.mainBot.sock && global.mainBot.sock.ws ? global.mainBot.sock.ws.readyState : 'N/A'
        };
        
        res.json({ 
          success: false, 
          message: 'Bot is not actually connected',
          status: 'disconnected',
          debug: debugInfo
        });
      }
...
app.post('/api/pm2/restart-bot', requireAuth, async (req, res) => {
  try {
    console.log('🔄 Restarting WhatsApp bot service...');
    console.log('🔧 Web server working directory:', process.cwd());
    console.log('🔧 Web server __dirname:', __dirname);
    
    // Send immediate response to client
    res.json({
      success: true,
      message: 'Restart process initiated. Service will be started.',
      note: 'The service will be stopped first, then the monitor will restart it.'
    });
    
    // Step 1: Stop the service first
    console.log('⏹️ Step 1: Starting whatsapp_api service...');
    try {
      const { exec } = require('child_process');
      const util = require('util');
      const execAsync = util.promisify(exec);
      ...
      // Stop the service
      try {
        await execAsync(`pm2 start ${processName}`, { cwd: projectRoot });
        console.log('✅ Service restarted');
      } catch (restartError) {
        console.log('⚠️ Restart error (may not exist):', restartError.message);
      }
    } catch (stopError) {
    ...
    // Ensure we send a resp  onse even on error
    if (!res.headersSent) {
      res.status(500).json({
        success: false,
        error: 'Failed to restart service',
        details: error.message,
        suggestion: 'Check if PM2 and the monitor script are available'
      });
    }
  } 
});

// PM2 Stop Bot Service
app.post('/api/pm2/stop-bot', requireAuth, async (req, res) => {
  try {
    console.log('⏹️ Stopping WhatsApp bot service via PM2...');
    
    // Check if process exists first
    const listResult = await execAsync('pm2 list --format json');
    const pm2List = JSON.parse(listResult.stdout);
    const whatsappProcess = pm2List.find(proc => 
      proc.name === 'whatsapp_api' || 
      proc.name === 'whatsapp-ai-bot' || 
      proc.name === 'fixed-whatsapp-bot'
    );
    
    if (!whatsappProcess) {
      return res.status(404).json({
        success: false,
        error: 'WhatsApp bot process not found in PM2',
        message: 'No WhatsApp bot process is currently running in PM2'
      });
    }
    
    const processName = whatsappProcess.name;
    console.log(`📱 Found process: ${processName}`);
    
    const stopResult = await execAsync(`pm2 stop ${processName}`);
    console.log('✅ Stop result:', stopResult.stdout);
    
    res.json({
      success: true,
      message: `WhatsApp bot service (${processName}) stopped successfully`,
      processName: processName,
      output: stopResult.stdout
    });
    
  } catch (error) {
    console.error('❌ Error stopping PM2 service:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to stop PM2 service',
      details: error.message
    });
  }
});

// PM2 Start Bot Service
app.post('/api/pm2/start-bot', requireAuth, async (req, res) => {
  try {
    console.log('▶️ Starting WhatsApp bot service via PM2...');
    
    const path = require('path');
    const projectRoot = path.resolve(__dirname, '..');
    
    //     const PROJECT_ROOT="/home/whatsapp"
    // PROCESS_NAME="whatsapp_api"

    console.log("projectRoot", projectRoot);
    // Check if process exists first
    const listResult = await execAsync('pm2 list --format json', { cwd: projectRoot });
    const pm2List = JSON.parse(listResult.stdout);
    const whatsappProcess = pm2List.find(proc => 
      proc.name === 'whatsapp_api' || 
      proc.name === 'whatsapp-ai-bot' || 
    ...
    // Try to restart instead of start if already running
    const startResult = await execAsync(`pm2 restart ${processName}`, { cwd: projectRoot });
    console.log('✅ Start result:', startResult.stdout);
    
    res.json({
      success: true,
      message: `WhatsApp bot service (${processName}) restarted successfully`,
      processName: processName,
      output: startResult.stdout
    });
    
  } catch (error) {
    console.error('❌ Error starting PM2 service:', error);
    res.status(500).json({
      success: false,
...
app.get('/api/pm2/status', requireAuth, async (req, res) => {
  try {
    ...
    let pm2Version = null;
    try {
      const versionResult = await execAsync('pm2 --version');
      pm2Version = versionResult.stdout.trim();
    } catch (versionError) {
      console.error('❌ PM2 version check failed:', versionError);
    }
    
    const statusResult = await execAsync('pm2 list');
    
    const jlistResult = await execAsync('pm2 jlist');
    const pm2List = JSON.parse(jlistResult.stdout);
    const whatsappApiProcess = pm2List.find(proc => 
      proc.name === 'whatsapp_api' || 
      proc.name === 'whatsapp-ai-bot' || 
      proc.name === 'fixed-whatsapp-bot' ||
      proc.name.includes('whatsapp') ||
      proc.name.includes('bot')
    );
    ...
    res.json({
      ...
      allProcesses: pm2List.map(p => ({
        name: p.name,
        status: p.pm2_env?.status || 'unknown',
        uptime: p.pm2_env?.uptime || 0
      })),
      whatsappApi: whatsappApiProcess ? {
        name: whatsappApiProcess.name,
        status: whatsappApiProcess.pm2_env.status,
        uptime: whatsappApiProcess.pm2_env.uptime,
        restarts: whatsappApiProcess.pm2_env.restart_time,
        cpu: whatsappApiProcess.monit.cpu,
        memory: whatsappApiProcess.monit.memory
      } : null
    });
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to check PM2 status',
      details: error.message
    });
  }
});

// Function to trigger QR generation (manual only)
function triggerQRGeneration() {
  try {
    ...
    console.log('🔄 Manually requesting QR generation from bot process...');
    ...
    if (fs.existsSync(connectionStateFile)) {
      try {
        ...
        if (stateAge < 300000) { // 5 minutes
          ...
          console.log('📱 Not connected; QR generation will be triggered manually');
        } else {
...
// Enhanced QR generation with PM2 restart fallback
async function generateQRWithFallback() {
  try {
    console.log('🔄 Attempting QR generation with fallback...');
    
    // First try: Use bot instance if available
    if (global.mainBot && typeof global.mainBot.forceQRGeneration === 'function') {
      console.log('🤖 Using bot instance for QR generation...');
      const result = await global.mainBot.forceQRGeneration();
      if (result) {
        console.log('✅ QR generation via bot instance successful');
        return { success: true, method: 'bot-instance' };
      }
    }
    
    // Second try: Clear session and restart PM2
    console.log('🔄 Bot instance method failed, trying PM2 restart...');
    const { exec } = require('child_process');
    const util = require('util');
    const execAsync = util.promisify(exec);
    
    // Stop PM2 service
    console.log('⏹️ Stopping PM2 service...');
    await execAsync('pm2 stop whatsapp_api');
    ...
    // Start PM2 service
    console.log('▶️ Starting PM2 service...');
    await execAsync('pm2 start whatsapp_api');
    
    console.log('✅ PM2 restart completed, QR should be generated');
    return { success: true, method: 'pm2-restart' };
    
  } catch (error) {
    console.error('❌ QR generation with fallback failed:', error);
...
app.post('/api/whatsapp/disconnect', requireAuth, (req, res) => {
  try {
    ...
    // Method 1: Try to access the bot instance from the main process
    if (global.mainBot && global.mainBot.sock) {
      console.log('🔌 Disconnecting via global bot instance...');
      global.mainBot.sock.logout().then(() => {
        ...
        console.log('📝 Connection state updated to disconnected');
      }).catch(error => {
        console.error('❌ Error disconnecting WhatsApp instance:', error);
      });
    } else {
      console.log('⚠️ Bot instance not available, clearing session data...');
      
      // Method 2: Clear session data to force disconnect
      const fs = require('fs');
      const path = require('path');
      ...
      console.log('✅ Session cleared and state updated to disconnected');
    }
...
io.on('connection', (socket) => {
  ...
  // Handle connection status updates from bot
  socket.on('connectionStatusUpdate', (data) => {
    console.log('📡 Received connection status update:', data.status);
    // Broadcast to all connected clients
    io.emit('connectionStatusUpdate', data);
  });
...
// API endpoint to force refresh connection status
app.post('/api/connection-status/refresh', requireAuth, (req, res) => {
  try {
    console.log('🔄 Force refreshing connection status...');
    // If bot is available, get real-time status
    if(global.mainBot) {
      const botStatus = global.mainBot.getConnectionStatus();
      console.log('🤖 Bot status:', botStatus);
      // Force update connection state if bot is connected
      if (global.mainBot.isConnected && botStatus.isConnected) {
        const connectedNumber = global.mainBot.sock?.user?.id?.replace('@c.us', '') || null;
        global.mainBot.sessionManager.saveConnectionState({
        ...
        // Emit status update
        global.mainBot.emitConnectionStatus('connected');
        
        return res.json({ 
          success: true, 
          status: 'connected', 
          connectedNumber: connectedNumber,
          message: 'Bot is connected and status updated' 
        });
      }
    }
    
    if (global.mainBot && global.mainBot.isConnected) {
      const realTimeState = {
        state: 'connected',
        isReady: true,
        isAuthenticated: true,
        connectedNumber: global.mainBot.sock?.user?.id || null,
        lastHeartbeat: new Date().toISOString(),
        lastUpdate: new Date().toISOString(),
      ...
      fs.writeFileSync(stateFile, JSON.stringify(realTimeState, null, 2));
      
      // Emit status update to dashboard
      if (global.mainBot.emitConnectionStatus) {
        global.mainBot.emitConnectionStatus('connected');
      }
      
      console.log('✅ Connection status refreshed to CONNECTED');
      return res.json({ success: true, status: 'connected', message: 'Status refreshed successfully' });
    } else {
      console.log('⚠️ Bot not connected, cannot refresh status');
      return res.json({ success: false, status: 'disconnected', message: 'Bot not connected' });
    }
  } catch (error) {
...
// API endpoint to get connection status with detailed info (no auth required for status)
    ...
    res.json({
      ...
      connectionState: {
        ...
        qrData: qrData ? {
          ...
          maxRetries: qrData.maxRetries || 10,
