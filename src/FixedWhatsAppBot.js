...
class FixedWhatsAppBot extends EventEmitter {
    ...
    async initialize() {
        try {
            // Check if we're already connecting (but allow manual QR requests)
            if (this.isConnecting && !this.manualQRRequested) {
            ...
            // Reset manual QR request flag after successful initialization
            this.manualQRRequested = false;
    ...
    setupSocketEventHandlers(saveCreds) {
        this.sock.ev.on('connection.update', async (update) => {
            ...
            this.reconnectTimeout = setTimeout(() => {
                ...
                // Save connection state with detailed logging
                const connectedNumber = this.sock?.user?.id?.replace('@c.us', '') || null;
                console.log(`📱 Saving connection state for: ${connectedNumber}`);
                ...
                this.sessionManager.saveConnectionState(connectionStateData);
                
                // Verify state was saved
                const savedState = this.sessionManager.getConnectionState();
                console.log(`💾 Verified saved state: ${savedState.connectionState} (${savedState.connectedNumber})`);
        ...
        try {
            ...
            if (io) {
                ...
                const connectionData = {
                    ...
                    connectedNumber: status === 'connected' ? (this.sock?.user?.id?.replace('@c.us', '') || state.connectedNumber) : null,
                    lastHeartbeat: state.lastHeartbeat || new Date().toISOString(),
                    connectionState: state,
                    timestamp: new Date().toISOString()
                };
                
                io.emit('connectionStatusUpdate', connectionData);
                io.emit('connectionStateChanged', connectionData);
                console.log(`📡 Emitted connection status: ${status} (${statusMessage})`);
            }
        } catch (error) {
    ...
    async handleManualQRRequest() {
        try {
            ...
            // Reset connection flags to allow fresh initialization
            this.isConnecting = false;
            this.lastReconnectTime = 0;
