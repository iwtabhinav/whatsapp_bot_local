...
class UltraRobustWhatsAppBot extends EventEmitter {
    constructor(config = {}) {
        ...
        this.whitelistInitialized = false; // Track if whitelist has been initialized
        ...
        this.isReconnecting = false; // Flag to prevent multiple simultaneous reconnections
    ...
    async initializeWhitelistedNumbers() {
        try {
            ...
            this.whitelistInitialized = true; // Mark as initialized
        } catch (error) {
        ...
        whitelistedNumbers.forEach(number => {
            ...
            this.whitelistInitialized = true; // Mark as initialized even with fallback
        }
    }
    ...
    // Helper function to extract phone number from JID (handles both @c.us and @s.whatsapp.net formats)
    extractPhoneNumber(jid) {
        if (!jid) return '';
        // Handle both @c.us and @s.whatsapp.net formats
        // Also handle device IDs like :91@s.whatsapp.net (extract only the phone number part)
        let phoneNumber = jid;
        if (jid.includes('@')) {
            phoneNumber = jid.split('@')[0];
        }
        // Remove device ID if present (e.g., "971563905407:91" -> "971563905407")
    ...
    shouldProcessMessage(msg) {
        ...
        // Extract phone number - handle both @c.us and @s.whatsapp.net formats
    ...
    isBookingRelatedMessage(messageText) {
        ...
        const bookingKeywords = [
            ...
            'chauffeur', 'taxi', 'ride', 'driver', 'pickup', 'drop',
            'pick up', 'drop off', 'airport', 'transfer', 'hourly',
            'sedan', 'suv', 'luxury', 'van', 'vehicle', 'car',
            'passenger', 'luggage', 'location', 'address',
            'confirm', 'payment', 'pay', 'price', 'cost', 'fare',
            'book now', 'start booking', 'new booking', 'make booking',
            'book chauffeur', 'book taxi'
        ];
        ...
        // Check for list selection patterns (booking-related selections)
        const listSelectionPatterns = [
            'book now', 'voice booking', 'image booking', 'view services',
            'hourly booking', 'transfer booking', 'sedan', 'suv',
             'luxury', 'van', 'book chauffeur', 'book taxi'
        ];

        for (const pattern of listSelectionPatterns) {
            if (text.includes(pattern)) {
                return true;
            }
        }

        // Check for location-related messages (likely booking-related)
        if (text.match(/\b(location|address|where|pickup|drop|destination)\b/)) {
            return true;
        }
    ...
    validateInput(messageText, step, bookingType) {
        ...
        switch (step) {
            case 1: // Booking type - COMMENTED OUT: Now defaulted to "Hourly Booking", validation not needed
                // Booking type is now defaulted to "Hourly Booking", so always return valid
                // If somehow we get here, default to Hourly Booking
    ...
    async initialize() {
        if (this.isConnecting || this.isReconnecting) {
            console.log('⏳ Already connecting or reconnecting, please wait...');
            return;
        }
        ...
        try {
            ...
            if (this.sock.ws) {
                this.sock.ws.on('error', (error) => {
                    ...
                    if (error.message.includes('WebSocket was closed')) {
                        console.log('🔄 WebSocket closed, will retry connection...');
                        this.isConnected = false;
                        this.isConnecting = false;
                    }
                });
            ...
            // Always reset the reconnection flag
    ...
    setupSocketEventHandlers() {
        ...
        this.sock.ev.on('connection.update', (update) => {
            ...
            if (qr) {
                ...
                if (this.autoGenerateQR && this.lastQRCode !== qr && !this.isGeneratingQR) {
                    ...
                    this.qrCodeGenerated = true; // Mark QR as generated
                    ...
                    try {
                        ...
                        if (io) {
                            ...
                            qrcode.toDataURL(qr, {
                                ...
                                // Update QR data with current attempt count
                                const qrData = {
                                    qrCode: qrDataURL,
                                    qrData: qr,
                                    timestamp: new Date().toISOString(),
                                    attempt: this.connectionAttempts, // Use connection attempts, not QR attempts
                                    maxRetries: this.maxConnectionAttempts,
                                    age: 'fresh'
                                };
                                
                                // Save updated QR data to file
                                const qrFilePath = path.join(__dirname, '../data/whatsapp-qr.json');
                                const dataDir = path.dirname(qrFilePath);
            ...
            if (connection === 'close') {
                ...
                // Handle WebSocket closure errors specifically
                if (errorMessage.includes('WebSocket was closed')) {
                    console.log('⚠️ WebSocket closure detected, increasing delay...');
                    this.connectionAttempts = Math.min(this.connectionAttempts + 2, 10); // Increase attempts for WebSocket errors
                }
                ...
                if (shouldReconnect) {
                    ...
                    // Increase delay for WebSocket errors
                    const baseDelay = errorMessage.includes('WebSocket was closed') ? 15000 : 10000;
                    const delay = Math.min(baseDelay + (this.connectionAttempts * 5000), 30000); // 15s base for WebSocket errors
                ...
                this.emitWebSocketEvent('connectionStateChanged', {
                    ...
                    connectedNumber: this.extractPhoneNumber(this.sock?.user?.id) || 'Unknown',
                    timestamp: new Date().toISOString()
                });
        ...
        this.sock.ev.on('messages.upsert', async (m) => {
            ...
            // Log raw JID for debugging
            console.log(`📨 Raw message JID: ${msg.key.remoteJid}`);
            console.log(`📱 Extracted phone number: ${phoneNumber}`);
            
            // Check if whitelist is initialized
            if (!this.whitelistedNumbers || this.whitelistedNumbers.size === 0) {
                console.log(`⚠️ Whitelist not initialized! Initializing now...`);
                await this.initializeWhitelistedNumbers();
                this.startWhitelistRefresh();
                this.whitelistInitialized = true;
            }

            // Check if message should be processed (deduplication and rate limiting)
            if (!this.shouldProcessMessage(msg)) {
                console.log(`⚠️ Message from ${phoneNumber} skipped by shouldProcessMessage`);
                return;
            }
            ...
            if (!this.isWhitelisted(phoneNumber)) {
                ...
                console.log(`📋 Whitelisted numbers (${this.whitelistedNumbers.size}): ${Array.from(this.whitelistedNumbers).join(', ')}`);
                const cleanedNumber = phoneNumber.replace(/[^\d+]/g, '').replace(/^\+/, '');
                console.log(`🔍 Cleaned number for comparison: ${cleanedNumber}`);
                console.log(`🔍 Is cleaned number in whitelist? ${this.whitelistedNumbers.has(cleanedNumber)}`);
                return;
            }
    ...
    emitConnectionStatus(status) {
        try {
            ...
            const connectionState = {
                ...
                connectedNumber: status === 'connected' ? this.extractPhoneNumber(this.sock.user?.id) : null,
    ...
    // Force status synchronization - check if bot is actually connected
    async forceStatusSync() {
        try {
            console.log('🔄 Forcing status synchronization...');
            console.log('🔍 Debug info:');
            console.log('  - this.sock exists:', !!this.sock);
            console.log('  - this.isConnected:', this.isConnected);
            console.log('  - this.sock.user exists:', !!(this.sock && this.sock.user));
            console.log('  - this.sock.user.id exists:', !!(this.sock && this.sock.user && this.sock.user.id));
            console.log('  - WebSocket readyState:', this.sock && this.sock.ws ? this.sock.ws.readyState : 'N/A');
            
            // Check if bot is actually connected
            if (this.isConnectionHealthy()) {
                console.log('✅ Bot is actually connected, updating status...');
                this.isConnected = true;
                this.isConnecting = false;
                this.qrCodeGenerated = false;
                this.connectionAttempts = 0;
                
                // Save the correct connection state
                this.saveConnectionState('connected');
                
                // Emit the correct status
                this.emitConnectionStatus('connected');
                
                console.log('✅ Status synchronized: Bot is CONNECTED');
                return true;
            } else {
                console.log('❌ Bot is not actually connected');
                console.log('🔍 Connection health check failed - bot needs to reconnect');
                
                // Force the bot to update its status to disconnected
                this.isConnected = false;
                this.isConnecting = false;
                this.saveConnectionState('disconnected');
                this.emitConnectionStatus('disconnected');
                
                console.log('🔄 Bot status updated to disconnected - dashboard should now show correct status');
                return false;
            }
        } catch (error) {
    ...
    saveConnectionState() {
        try {
            ...
            const connectionState = {
                ...
                connectedNumber: this.extractPhoneNumber(this.sock?.user?.id) || null,
                lastHeartbeat: now,
                lastUpdate: now,
    ...
    isConnectionHealthy() {
        try {
            ...
            if (!this.sock || !this.isConnected) {
                console.log('🔍 Connection health: Socket not connected');
                return false;
            }

            // Check if we have a valid user ID (this is the key indicator of successful authentication)
            if (!this.sock.user || !this.sock.user.id) {
                console.log('🔍 Connection health: No valid user ID');
                return false;
            }

            // Check if the connection is not in an error state
            if (this.sock.ws && this.sock.ws.readyState !== 1) { // 1 = OPEN
                console.log('🔍 Connection health: WebSocket not ready');
                return false;
            }

            // Additional check: verify session is valid
            if (this.sessionValidator && !this.sessionValidator.isSessionValid()) {
                console.log('🔍 Connection health: Session not valid');
                return false;
            }
    ...
    async processMessage(msg) {
        try {
            ...
            // Handle location messages - only if there's an active booking session
            if (msg.message.locationMessage) {
                const session = this.bookingManager.getActiveSession(phoneNumber);
                if (session) {
                    await this.handleLocationMessage(msg, phoneNumber);
                } else {
                    console.log(`⚠️ Location message received but no active booking session for ${phoneNumber}, ignoring...`);
                }
                return;
            }
            ...
            if (msg.message.imageMessage || msg.message.videoMessage || msg.message.audioMessage) {
                ...
                if (processedMedia) {
                    ...
                    // Check if processed media message is booking-related
                    const isBookingRelated = this.isBookingRelatedMessage(messageText);
                    ...
                    if (session) {
                        ...
                        // Media is booking-related but no session - continue to create session below
                        // Don't return here, let it fall through to booking creation logic
                    } else {
                        // Media is not booking-related and no session - ignore it
                        console.log(`⚠️ Non-booking media message from ${phoneNumber} ignored`);
                        return;
                    }
                } else {
                    // If media processing didn't return text, check if there's an active session
                    const session = this.bookingManager.getActiveSession(phoneNumber);
                    if (session) {
                        // If there's a session, acknowledge the media
                        await this.sendMessage(msg.key.remoteJid, 'I received your media message. Please send a text message if you need assistance.');
                    } else {
                        // No session and media couldn't be processed - ignore it
                        console.log(`⚠️ Media message from ${phoneNumber} couldn't be processed and no active session, ignoring...`);
                    }
                    return;
                }
            }
            ...
            console.log(`🔍 Booking related check: "${messageText}" -> ${isBookingRelated ? 'YES' : 'NO'}`);
            ...
            // If session exists and user sends booking-related message, continue with booking
            if (session && (isBookingRelated || isListSelection)) {
                console.log(`📋 User sent booking-related message, continuing with existing session: ${session.bookingId}`);
            ...
            if (existingSession && existingSession.status === 'confirmed' && isBookingRelated) {
            ...
            // Only create new session if no session exists and message is booking-related
            if ((isBookingRelated || isListSelection || (msg.message.imageMessage && messageText.toLowerCase().includes('need'))) && !session) {
                ...
                // IMPORTANT: Return here to wait for user to select from dropdown
                // Don't continue processing - wait for user's selection from the dropdown
                console.log(`✅ Welcome message sent, waiting for user selection...`);
                return;
            }
            ...
            if (session) {
                await this.processBookingStep(msg, phoneNumber, messageText, session);
            } else if (isBookingRelated || isListSelection) {
                // Message is booking-related but no session exists - this should have been handled above
                // But if we reach here, it means the message is booking-related, so we should respond
                console.log(`📋 Booking-related message detected but no session, creating session...`);
                
                // This case should be rare, but handle it by creating a session
                const contextType = messageText.toLowerCase().includes('airport') ? 'airport' : 'chauffeur';
                const bookingId = this.bookingManager.createSession(phoneNumber, contextType);
                session = this.bookingManager.sessions[bookingId];
                this.analytics.bookingsCreated++;
                ...
                // Message is NOT booking-related and no active session
                // Ignore the message - don't reply to non-booking messages
                console.log(`⚠️ Non-booking message from ${phoneNumber} ignored: "${messageText.substring(0, 50)}..."`);
                // Optionally, you can send a simple message asking if they want to book
                // Uncomment the line below if you want to send a message for non-booking messages
                // await this.sock.sendMessage(msg.key.remoteJid, {
                //     text: "Hello! I'm the VIP Chauffeur booking assistant. To book a ride, please send a message with 'book' or 'booking' in it, or use the menu options."
                // });
            }
        ...
        try {
            ...
            // Handle list responses (both interactiveResponseMessage.listReply and listResponseMessage)
                ...
                if (listId === 'book_now' || (listId && listId.includes('Book Now'))) {
                    ...
                    // COMMENTED OUT: Booking type editing is disabled - defaults to "Hourly Booking"
                    // Mark session as editing and set the field being edited
                    const session = this.bookingManager.getActiveSession(phoneNumber);
                    console.log(`🔧 Edit booking type selected for session:`, session?.bookingId);
                    if (session) {
                        // Auto-set to Hourly Booking (booking type menu is commented out)
                        this.bookingManager.updateSession(session.bookingId, null, 'system', {
                            bookingType: 'Hourly Booking',
                            isEditing: false,
                            editingField: null
                        });
                        console.log(`✅ Booking type set to Hourly Booking (default)`);
                        await this.sock.sendMessage(msg.key.remoteJid, {
                            text: "✅ Booking type has been set to Hourly Booking (default)."
                        });
                        // Show edit options
                        await this.showEditOptions(msg.key.remoteJid, phoneNumber);
                    }
                    // COMMENTED OUT: await this.showBookingTypeMenu(msg.key.remoteJid, phoneNumber);
    ...
    async startBookingProcess(msg, phoneNumber) {
        ...
        if (!session) {
            ...
            // Initialize current step to 1 (booking type selection)
            this.bookingManager.updateSession(bookingId, null, 'system', { 
                currentStep: 1
            });
            ...
            console.log(`📋 New session created: ${bookingId}, current step: 1 (booking type selection)`);
            ...
            // If session exists but no current step, show booking type menu
    ...
    async showBookingTypeMenu(jid, phoneNumber) {
        ...
        const stepNumber = 1;
        // Calculate total steps - use Hourly Booking as default for display (it has more steps)
        const totalSteps = this.getTotalSteps('Hourly Booking');
    ...
    // Step 2: Show vehicle type menu (after booking type selection)
    async showVehicleTypeMenu(jid, phoneNumber) {
        // Get the session to determine booking type and calculate correct step numbers
        const session = this.bookingManager.getActiveSession(phoneNumber);
        const bookingType = session ? (session.data.bookingType || 'Hourly Booking') : 'Hourly Booking';
        
        // Vehicle type is step 2 (after booking type selection)
        const stepNumber = this.getDisplayStepNumber('vehicleType', bookingType);
        // Calculate total steps based on booking type (including bookingType field)
    ...
    async handleVehicleSelection(msg, phoneNumber, vehicleType) {
        ...
        if (session.data.isEditing && session.data.editingField === 'vehicleType') {
            ...
            // Regular booking flow
            // Step 3: customerName (step 1 is bookingType, step 2 is vehicleType)
            this.bookingManager.updateSession(session.bookingId, null, 'system', {
                vehicleType: vehicleType,
                currentStep: 3  // Step 3: customerName (after bookingType=1, vehicleType=2)
            });
    ...
    async extractBookingInfo(messageText, session) {
        try {
            ...
            const currentStep = session.data.currentStep || 1; // Default to step 1 (booking type selection)
...
Return only valid JSON, no other text.
    ...
    async processBookingStep(msg, phoneNumber, messageText, session) {
        try {
            ...
            if (isEditing && editingField) {
                ...
                if (validation.valid) {
                    // Update the field
                    // Ensure passengerCount is saved as string for consistency
                    const fieldValue = nextField === 'passengerCount' && typeof validation.value !== 'string' 
                        ? validation.value.toString() 
                        : validation.value;
                    
                    this.bookingManager.updateSession(session.bookingId, null, 'system', {
                        [nextField]: fieldValue
                    });

                    console.log(`✅ Updated ${nextField}: ${fieldValue} (type: ${typeof fieldValue})`);
    ...
    getRequiredFieldsForBookingType(bookingType) {
        const baseFields = ['bookingType', 'vehicleType', 'customerName', 'pickupLocation'];
        const commonFields = ['luggageInfo', 'passengerCount', 'specialRequests'];

        if (bookingType === 'Transfer Booking') {
            // For Transfer Booking: pickupLocation -> dropLocation -> luggageInfo -> passengerCount -> specialRequests
            return [...baseFields, 'dropLocation', ...commonFields];
        } else if (bookingType === 'Hourly Booking') {
            // For Hourly Booking: pickupLocation -> numberOfHours -> luggageInfo -> passengerCount -> specialRequests
            return [...baseFields, 'numberOfHours', ...commonFields];
        }

        return [...baseFields, ...commonFields];
    }

    // Get missing fields for current session
    getMissingFieldsForSession(session) {
        const data = session.data;
        const bookingType = data.bookingType || 'Hourly Booking'; // Default if not set
        const requiredFields = this.getRequiredFieldsForBookingType(bookingType);

        console.log(`🔍 Checking missing fields for ${bookingType}:`, requiredFields);
        console.log(`📊 Current session data:`, Object.keys(data).map(key => `${key}: ${data[key]}`).join(', '));

        // Include 'bookingType' in missing fields check (it's step 1)
        const missingFields = requiredFields.filter(field => {
            ...
            // For string values, check if empty or "Not specified"
            if (typeof value === 'string') {
                return value.trim() === '' || value === 'Not specified';
            }
            
            // For number values, check if it's 0 or invalid
            if (typeof value === 'number') {
                return value === 0 || isNaN(value);
            }
            
            // If value exists and is valid, field is not missing
            return false;
        });

        console.log(`📋 Missing fields (in order):`, missingFields);
    ...
    getDisplayStepNumber(field, bookingType) {
        const requiredFields = this.getRequiredFieldsForBookingType(bookingType);
        // Include 'bookingType' in step calculation (it's step 1)
        const fieldIndex = requiredFields.indexOf(field);
        return fieldIndex + 1; // 1-based indexing (bookingType is step 1, vehicleType is step 2, etc.)
    }

    // Get total steps for booking type
    getTotalSteps(bookingType) {
        const requiredFields = this.getRequiredFieldsForBookingType(bookingType);
        // Include 'bookingType' in count (it's step 1)
        ...
        try {
            ...
            if (session) {
                ...
                if (missingFields.includes('pickupLocation')) {
                    ...
                    // Get updated session to check next missing field
                    const updatedSession = this.bookingManager.sessions[session.bookingId];
                    const stillMissing = this.getMissingFieldsForSession(updatedSession);

                    // Continue with booking flow - ask for next missing field
                    if (stillMissing.length > 0) {
                        const nextField = stillMissing[0];
                        console.log(`📍 Pickup location set, next field to ask: ${nextField}`);
                        await this.askForMissingField(msg, phoneNumber, nextField, updatedSession.data.bookingType);
                    } else {
                    ...
                    if (stillMissing.length > 0) {
                        const nextField = stillMissing[0];
                        console.log(`📍 Drop location set, next field to ask: ${nextField}`);
                        await this.askForMissingField(msg, phoneNumber, nextField, updatedSession.data.bookingType);
                    } else {
                        // All fields collected, show confirmation
                        await this.showBookingConfirmation(msg.key.remoteJid, phoneNumber);
...
This bot has ultra-robust connection handling and AI-powered booking!`;
    ...
    async forceQRGeneration() {
        try {
            ...
            // Reset QR tracking variables to ensure fresh generation
            ...
            // Clear QR file to prevent showing old QR
            const qrFilePath = path.join(__dirname, '../data/whatsapp-qr.json');
            if (fs.existsSync(qrFilePath)) {
                try {
                    fs.unlinkSync(qrFilePath);
                    console.log('🗑️ Old QR file cleared');
                } catch (e) {
                    console.log('⚠️ Error clearing QR file:', e.message);
                }
            }
            ...
            // Force session validator to clear and prepare for new QR
            if (this.sessionValidator) {
                this.sessionValidator.forceQRGeneration();
            }
            
            // Wait longer before reinitializing to prevent WebSocket conflicts
            console.log('⏳ Waiting for connection to fully close...');
            await new Promise(resolve => setTimeout(resolve, 8000));
            
            // Force complete cleanup before reinitializing
            this.cleanup();
            
            // Wait a bit more after cleanup
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            // Reinitialize to trigger new QR generation
            console.log('🔄 Reinitializing bot to generate new QR...');
            await this.initialize();
