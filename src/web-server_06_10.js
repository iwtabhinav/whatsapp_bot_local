...
app.post('/api/whatsapp/qr/generate', requireAuth, async (req, res) => {
  try {
    ...
    // Clear session files to force QR generation on next bot restart
    ...
    if (result.success) {
      ...
      return res.status(500).json({
        success: false,
        error: result.error,
        details: result.details || 'QR generation completed but file not accessible'
      });
    }
    ...
    res.json({
      ...
      connectionState: {
        ...
        qrData: qrData ? {
          ...
          maxRetries: qrData.maxRetries || 10000,
