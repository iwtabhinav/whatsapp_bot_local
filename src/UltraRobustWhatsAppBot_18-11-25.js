...
class UltraRobustWhatsAppBot extends EventEmitter {
    ...
    setupSocketEventHandlers() {
        ...
        this.sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'close') {
                ...
                // Verify that the socket is actually authenticated before marking as connected
                // Wait a moment for user object to be populated after connection opens
                setTimeout(() => {
                    if (!this.sock || !this.sock.user || !this.sock.user.id) {
                        console.log('⚠️ Connection opened but user not authenticated yet. Waiting...');
                        // Check again after a short delay
                        setTimeout(() => {
                            if (!this.sock || !this.sock.user || !this.sock.user.id) {
                                console.log('❌ Connection opened but authentication failed or incomplete');
                                console.log('🔄 This may indicate QR scan failed. Will wait for new QR code...');
                                this.isConnected = false;
                                this.isConnecting = false;
                                this.emitConnectionStatus('qr_required');
                                return;
                            } else {
                                // User object now exists, proceed with connection
                                this.finalizeConnection();
                            }
                        }, 2000);
                    } else {
                        // User object exists immediately, proceed with connection
                        this.finalizeConnection();
                    }
                }, 500);
    ...
    // Finalize connection after authentication is verified
    finalizeConnection() {
        if (!this.sock || !this.sock.user || !this.sock.user.id) {
            console.log('❌ Cannot finalize connection: User not authenticated');
            this.isConnected = false;
            this.isConnecting = false;
            this.emitConnectionStatus('qr_required');
            return;
        ...
        console.log(`📱 Authenticated as: ${this.extractPhoneNumber(this.sock.user.id)}`);
        ...
        this.emitWebSocketEvent('connectionStateChanged', {
            ...
            connectedNumber: this.extractPhoneNumber(this.sock.user.id),
            timestamp: new Date().toISOString()
        });
        ...
        try {
            ...
            if (selectionText.includes('Book Now')) {
                ...
                if (session) {
                    ...
                    // Show edit options
                    await this.showEditOptions(originalMsg.key.remoteJid, phoneNumber);
                }
                // COMMENTED OUT: await this.showBookingTypeMenu(originalMsg.key.remoteJid, phoneNumber);
            } else if (session) {
                // If we have a session, process as part of booking flow
    ...
    async startBookingProcess(msg, phoneNumber) {
        ...
        if (!session) {
            ...
            // If session exists but no current step, start with booking type selection
    ...
    async showBookingTypeMenu(jid, phoneNumber) {
        ...
        const stepNumber = 1;
        // Calculate total steps - both booking types have the same base fields
        // Hourly: bookingType, vehicleType, customerName, pickupLocation, numberOfHours, luggageInfo, passengerCount, specialRequests (8 fields)
        // Transfer: bookingType, vehicleType, customerName, pickupLocation, dropLocation, luggageInfo, passengerCount, specialRequests (8 fields)
        const totalSteps = 8; // Both have 8 required fields
    ...
    async handleBookingTypeSelection(msg, phoneNumber, bookingType) {
        ...
        if (session.data.isEditing && session.data.editingField === 'bookingType') {
            ...
            // Regular booking flow - Step 1 completed, move to Step 2 (vehicle selection)
            this.bookingManager.updateSession(session.bookingId, null, 'system', {
                bookingType: bookingType,
                currentStep: 2
            });

            console.log(`✅ Booking type selected: ${bookingType}, moving to step 2 (vehicle selection)`);
    ...
    async handleVehicleSelection(msg, phoneNumber, vehicleType) {
        ...
        if (session.data.isEditing && session.data.editingField === 'vehicleType') {
            ...
            // Step 3: customerName (step 1 was bookingType, step 2 was vehicleType)
            this.bookingManager.updateSession(session.bookingId, null, 'system', {
                vehicleType: vehicleType,
                currentStep: 3  // Step 3: customer name (after booking type and vehicle type)
            });
    ...
    async extractBookingInfo(messageText, session) {
        try {
            ...
            const currentStep = session.data.currentStep || 2; // Default to step 2 (vehicle selection) since bookingType is now defaulted
...
Return only valid JSON, no other text.
    ...
    getMissingFieldsForSession(session) {
        ...
        // Include 'bookingType' in missing fields check (it's now step 1)
    ...
    getDisplayStepNumber(field, bookingType) {
        ...
        return fieldIndex + 1; // 1-based indexing (bookingType is step 1, vehicleType is step 2, customerName is step 3, etc.)
    }
    ...
    async askForMissingField(msg, phoneNumber, field, bookingType) {
        ...
        switch (field) {
            case 'bookingType':
                // Show booking type selection menu
