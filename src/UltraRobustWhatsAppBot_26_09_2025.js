...
class BookingManager {
    ...
    createSession(phoneNumber, contextType = 'chauffeur') {
        ...
        console.log(`🆔 Generated booking ID: ${bookingId} (counter: ${this.metadata.lastBookingId})`);
...
class UltraRobustWhatsAppBot extends EventEmitter {
    constructor(config = {}) {
        ...
        this.maxConnectionAttempts = 10; // Increased from 3 to 10
        ...
        this.qrMinAge = 10000; // Minimum 10 seconds between QR generations
    ...
    async initialize() {
        ...
        if (!sessionStatus.isValid) {
            console.log(`⚠️ ${sessionStatus.reason}`);
            if (sessionStatus.needsQR) {
                console.log('🔄 Clearing invalid session and generating QR code...');
        ...
        try {
            ...
            if (this.connectionAttempts < this.maxConnectionAttempts) {
                ...
                console.log('🔄 Waiting 30 seconds before retrying with reset counter...');
                setTimeout(() => {
    ...
    setupSocketEventHandlers() {
        ...
        this.sock.ev.on('connection.update', (update) => {
            ...
            if (qr) {
                ...
                // Only process QR if it's different from the last one AND enough time has passed AND not already generating
                if (this.lastQRCode !== qr && timeSinceLastQR >= this.qrMinAge && !this.isGeneratingQR) {
        ...
        try {
            ...
            if (bookingIntent || isListSelection || (msg.message.imageMessage && messageText.toLowerCase().includes('need'))) {
                if (!session) {
                    ...
                    console.log(`📋 Active session already exists for ${phoneNumber}: ${session.bookingId}`);
                }
            }
